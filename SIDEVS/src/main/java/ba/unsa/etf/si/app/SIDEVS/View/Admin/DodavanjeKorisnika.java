...
public class DodavanjeKorisnika {
			...
			public void actionPerformed(ActionEvent arg0) {
				try {
					if (validirajPolja()) {
						...
						DodavanjeKorisnikaVM dkvm = new DodavanjeKorisnikaVM(_sesija);

						Boolean state = dkvm.kreirajKorisnika(ime.getText(), prezime.getText(), maticniBroj.getText(),
								brojTelefona.getText(), email.getText(), radnoMjesto.getText(),
								datumPocetkaRada.getText(), adresa.getText(), tipKorisnika);

						if (!state) {
							throw new Exception();
						}

						// Javlja da je korisnik kreiran ukoliko je sve prošlo
						// ok
	...
	public Boolean validirajPolja() throws WrongInputException {
		...
		if (ime.getText().isEmpty())
			msg = "Morate unijeti ime";
		else if (prezime.getText().isEmpty())
			msg = "Morate unijeti prezime";
		else if (!Validator.validirajMaticniBroj(maticniBroj.getText()))
			msg = "Uneseni JMBG nije ispravan";
		else if (!Validator.validirajTelefonskiBroj(brojTelefona.getText()))
			msg = "Telefonski broj nije ispravan";
		else if (adresa.getText().isEmpty())
			msg = "Morate unijeti adresu";
		else if (!Validator.validirajEmail(email.getText()))
			msg = "Neispravan email";
		else if (radnoMjesto.getText().isEmpty())
			msg = "Morate unijeti radno mjesto";
		else if (!Validator.isDateValid(datumPocetkaRada.getText()))
			msg = "Datum nije validan";
		else if (!radnik.isSelected() && !menadzer.isSelected())
			msg = "Morate odabrati radnu poziciju";
		if (msg != "") {
